...
### 论文的主要创新点是什么？解决了哪些问题？
- 这篇论文主要是研究实时人像视频抠图相关的课题，研究目标主要分为三个方面，时序稳定性、语义准确性、推理实时性。第一个研究点主要针对视频人像抠图的稳定性和准确性。通过引入ConvGRU，其循环状态向量可以记忆之前视频帧的信息，相对于单纯逐帧推理的算法显著提升了时序稳定性；通过引入深度估计分支和注意力机制，让神经网络去学习深度图与alpha matte之间的信息关联，引导网络更加关注深度较小的前景特征，相对于前言模型RVM有效抑制了背景无关人物的干扰，提升了语义准确性。从时间和空间两个维度的约束信息由本文所设计的TSCM模块进行交互和处理，总体上对各个场景都达到了良好的效果。第二个研究点主要针对模型的实时性推理，目前的算法大多数是直接下采样或者全局细化的模式，要么是造成边缘细节缺失，要么是严重影响模型推理性能。本文将静态抠图模型BGMv2中的补丁修复算法迁移到视频领域中，并加以改进。在细化模块中加入ConvGRU以维持二阶段优化的时序稳定性，并采用了自适应大小的补丁尺寸以及难样本挖掘目标函数，在MAD dtSSD指标上均有所提高，并且相对于全局细化算法，显著提高了推理性能。综合以上两个研究点，最后设计了一个实时人像抠图系统，满足常见场景下的稳定性、准确性和实时性的要求。

### 论文还存在哪些不足，未来可以从哪些方面改进？
- 第一点是模型的场景泛化能力还需要进一步提高，如论文中所展示的，在运动模糊、光照干扰、镜头突变等具有挑战性的case下，模型仍然具有一定的局限性。未来可以考虑引入更多的高质量抠图数据集和数据增强算法对模型进行训练，并且引入域适应性相关的策略进行优化改进。

- 第二点是模型相对于前沿模型RVM堆叠了比较多的参数，所以当输入在HD分辨率以上时，在普通CPU上还是难以达到十分流畅的效果。因此还是需要进一步精简网路模型，支持算法在普通计算平台上进行部署和应用。

- 第三点是背景合成效果有时候不太和谐。主要表现在色彩和结构两个方面，目前是采用单纯的图像合成方法，当前景和背景色调上不统一时，合成的图片可能存在不自然的感觉，另外在结构上如果前景大小尺寸不太合适的时候，可能会出现人物悬浮在水面上、空中等一些不真实的情况。未来可以利用图像和谐化相关技术对合成图进一步进行处理优化。

### 为什么选择这个人像视频抠图这个课题？
- 从实际需求来说，抠图问题在短视频内容创作等方面具有非常重要的作用，特别是在疫情期间，视频会议系统中也需要背景虚化、背景替换等技术需求，因此人像视频抠图课题具有重要的应用意义。另外。这个课题的来源是我们实验室自研会议系统BlueSide中的一个重要的模块，也就是实时背景去除，我们对这个问题也有一定的工作积累。前期我们调研了很多抠图相关的算法，比如MODNet，它是静态抠图模型中非常优秀的算法，但是当我们使用这个模型时出现了严重的闪烁现象，我们还实验了带有限制性条件的算法BGM，即在固定背景的情况下，额外输入一张纯背景图片作为先验信息，获得了比较好的效果，但是场景非常受限，后来使用了RVM 算法，在一些变化不大的场景中效果比较好，但是当远处背景有人走过时会对结果产生撕裂的影响。因此我们根据这些调研结果和实际测试经验，设计了自己的算法模型，以RVM作为baseline，解决了稳定性和背景干扰的场景问题，并且在此基础上为了提高推理性能，增加了补丁修复相关的研究内容，最终完成了这篇论文。

### Matting（抠图）和Segmentation（分割）问题很相似，他们的区别在哪里,分割已经很成熟了，为什么不用分割代替抠图？
- 分割的本质是分类，把每一个像素映射到不同的类别，分割所得到的mask是离散的。
- 抠图的本质既有分类，其中包含确定的前景，确定的背景，也有回归，即边缘部分的透明度。Matting的alpha结果是连续平滑的，可以运用到图像合成公式中。因此抠图问题更具有挑战性。
- 在设计网络结构时，我们采用了多级特征融合策略，深层的高级语义信息有利于提高分类准确性，低级细节信息有利于保留细节特征。为了使得结果更加精细化，把五级下采样改为了四级下采样，目的是为了更加关注浅层所保留的大量低级细节信息，这样能够使得alpha的边缘更加精细；

### 完成这篇论文的难点主要在哪里？
- 数据处理
  - 时序处理上比较繁琐，普通的网络模型训练是4个维度BCHW，但是本文由于增加了时序模型，逻辑上是5个维度BTCHW，因此在构建dataloder的时候需要对数据进行分片和特殊的维度变换，非常容易出错。
  - 测试场景的分类和设计，如何尽可能地测试到各种不同的常见的场景，以反映模型的鲁棒性，需要记录大量的测试图片和数据。
- 模型搭建
  - 各个模块之间的协同需要很长的调试工作，在写ConvGRU相关的模块时涉及到的维度处理操作比较复杂。
- 模型训练
  - 一次性使用4块16G显存的P100，训练一次需要两天，时间长，因此时间规划也特别需要注意。
