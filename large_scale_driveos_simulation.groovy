...
def fastbag_play(String enable_topics, String dispatcher_server_container_name, String bag_name){
    ...
    def docker_exec_command = """
        source /opt/plusai/setup.bash
        source /opt/ros/melodic/setup.bash
        fastbag_play -clock -envelope_timestamp --paths ${bag_name} \
            --hz=600 \
...
def start_driveos_node(String ros_ip, Inet4Address v3na_linux_remote){
    def local_send_dir = ""
    def local_scratch_dir = "/tmp/plusai_linux_perf"
    def script_timeout = 60
    def adu_exec_command  = """
...
def get_node_log_file(String node, Inet4Address v3na_linux_remote, String result_dir, String package_version, String vehicle_name){
    def local_send_dir = ""
    def local_scratch_dir = "/tmp/plusai_linux_perf"
    def script_timeout = 60
    ...
    timeout(time: 24, unit: 'HOURS') {
        timestamps {
            node("l4e-v3na-linux-perf-slaves") {
                ...
                String vehicle_name = "pdb-l4e-lab001"
                ...
                lock(v3na_linux_lock_name) {
                    try{
                        ...
                        stage("Run large scale simulation") {
                            ...
                            start_driveos_node(v3na_linux_server, v3na_linux_remote)
                            sh(script: "sleep 3s", returnStdout: true)
                            ...
                            for (node in node_list) {
                                echo "NODE ==> ${node}"
                                get_node_log_file(node, v3na_linux_remote, result_dir, package_version, vehicle_name)
                            }
